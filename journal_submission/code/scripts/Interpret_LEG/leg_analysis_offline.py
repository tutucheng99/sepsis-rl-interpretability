"""
LEG Analysis for Offline RL Models (d3rlpy)

Linearly Estimated Gradient (LEG) analysis for interpreting
offline RL policies trained with d3rlpy (BC, CQL, offline DQN).

Usage:
    python scripts/leg_analysis_offline.py --model results/models/bc_simple_reward.d3 --n_states 10
    python scripts/leg_analysis_offline.py --model results/models/cql_simple_reward.d3 --n_states 20
"""
...
class LEGAnalyzer_Offline:
    """Linearly Estimated Gradient analyzer for d3rlpy models."""
    ...
    def __init__(
        ...
        """
        Initialize LEG analyzer for offline RL models.

        Args:
            model: Trained d3rlpy model (BC, CQL, or offline DQN)
    ...
    def get_q_values(self, state: np.ndarray) -> np.ndarray:
        ...
        try:
            # Try to get Q-values for all actions
            q_values = np.array([
                self.model.predict_value(state, np.array([[a]]))[0]
                for a in range(self.action_dim)
            ])
        except (NotImplementedError, AttributeError):
            # BC doesn't have predict_value, use policy probabilities as proxy
            print("Warning: Model doesn't support Q-values, using policy-based ranking")
            action = self.model.predict(state)[0]
            # Create pseudo Q-values (highest for chosen action)
            q_values = np.array([
                1.0 if a == action else -abs(a - action) * 0.1
                for a in range(self.action_dim)
            ])
    ...
    def compute_saliency_scores(
        ...
        # Compute LEG gradient estimate for each action
        for action in range(self.action_dim):
...
class LEGVisualizer:
    ...
    def plot_saliency_heatmap(
        ...
        # Title
        ax.set_title('LEG Saliency Analysis for Sepsis Treatment Policy (Offline RL)',
    ...
    def plot_top_features_detailed(
        ...
        # Super title
        fig.suptitle(
            f'LEG Analysis - Action {selected_action} (IV={iv_bin}, VP={vp_bin})',
...
def load_offline_model(model_path: str):
    """Load trained d3rlpy model"""
    ...
    if model_path.suffix == '.d3':
        ...
        print(f"Loaded d3rlpy model from {model_path}")
    else:
        # Try BC then CQL
        try:
            model = DiscreteBC.from_json(str(model_path))
            print(f"Loaded BC model from {model_path}")
        except:
            model = DiscreteCQL.from_json(str(model_path))
...
def main():
    parser = argparse.ArgumentParser(description='LEG Analysis for Offline RL Models')
    parser.add_argument('--model', type=str, required=True,
                        help='Path to trained d3rlpy model (.d3 file)')
    ...
    parser.add_argument('--output_dir', type=str, default='results/figures/leg_offline',
                        help='Output directory for figures')
    args = parser.parse_args()

    print("=" * 60)
    print("LEG ANALYSIS FOR OFFLINE RL MODEL")
    print("=" * 60 + "\n")
    ...
    leg_analyzer = LEGAnalyzer_Offline(
        model=model,
